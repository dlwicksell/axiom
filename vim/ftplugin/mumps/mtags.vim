...
function! MTag(argtag)
  ...
  "It is valid in MUMPS/GT.M to have a label in the first line with a different
  "name than the name of the routine. This matches up with the tags file.
  if l:argtag =~ "^^" "an ^routine reference
    ...
    for l:dir in l:gtmlist "loop through the list, making a modified string
      ...
      let l:path .= l:dir . ";," "rebuild as a string, adding a search directive
    endfor
  ...
  if l:argtag !~ "\\^" "deal with a local label, in the routine we're in
    ...
    if l:routine =~ ".m" "we have the .m extension
      let l:length = strlen(l:routine)-2 "find the length without the extension
      let l:routine = strpart(l:routine, 0, l:length) "remove the .m extension
    endif

    let l:argtag = l:argtag . "^" . l:routine "build the label^routine reference
  endif
  ...
  try
    ...
    let l:routine = strpart(l:argtag, stridx(l:argtag, "^")+1) "strip off label
    let l:routine = tr(l:routine, "%", "_") "change to external format
